...
const databaseCalls = {
    ...
    sortedNodes: async(pageSize, pageNum, allowHidden) => {
        if (allowHidden) return await getPaginatedTable(SORTED_NODE_TABLE, pageSize, pageNum);
        return await getPaginatedFilteredTable(SORTED_NODE_TABLE, pageSize, pageNum, 'hidden', false);
    ...
    randomNode: async(max = 1000, allowHidden) => {
        ...
        if (allowHidden) {
            ...
            recentNodes = await getPaginatedFilteredTable(SORTED_NODE_TABLE, max, 0, 'hidden', false);
        }
    ...
    getAccount: async(accountScreenName) => await getItem(ACCOUNT_TABLE, { screenName: accountScreenName }),
    getNode: async(nodeID) => await getItem(NODE_TABLE, { ID: nodeID }),
    getChoice: async(choiceID) => await getItem(CHOICE_TABLE, { ID: choiceID }),
    getFeedback: async(feedbackID) => await getItem(FEEDBACK_TABLE, { ID: feedbackID }),

    addAccount: async(account) => await addItem(ACCOUNT_TABLE, account),
    addNode: async(node) => {
        await addItem(SORTED_NODE_TABLE, { idx: "Node", ID: node.ID, lastUpdated: node.lastUpdated || new Date(node.dateCreated).getTime(), hidden: node.hidden || node.pictureUnsafe || false, featured: node.featured || false, });
    ...
    removeAccount: async(accountScreenName) => await removeItem(ACCOUNT_TABLE, { screenName: accountScreenName }),
    removeNode: async(nodeID) => {
        await removeItem(NODE_TABLE, { ID: nodeID });
        await removeItem(SORTED_NODE_TABLE, { ID: nodeID });
    },
    removeChoice: async(choiceID) => await removeItem(CHOICE_TABLE, { ID: choiceID }),
    removeFeedback: async(feedbackID) => await removeItem(FEEDBACK_TABLE, { ID: feedbackID }),
};

const getPaginatedFilteredTable = async(tableName, pageSize, pageNumber, arg, value, lastKey, limit) => {
    if (!DBON) return [];
    ...
    if (items.Count < (limit || pageSize)) {
        if (pageNumber > 0) {
            ...
            return await getPaginatedFilteredTable(tableName, pageSize, pageNumber - 1, arg, value, items.LastEvaluatedKey);
        }
        if (items.LastEvaluatedKey === undefined) return items.Items;
        return [...items.Items,
            ...(await getPaginatedFilteredTable(tableName, pageSize, pageNumber, arg, value, items.LastEvaluatedKey, (limit || pageSize) - items.Count))
        ];
    }
...
const getPaginatedFilteredVisibleTable = async(tableName, pageSize, pageNumber, arg, value, lastKey, limit) => {
    if (!DBON) return [];
    ...
    if (items.Count < (limit || pageSize)) {
        if (pageNumber > 0) {
            if (items.LastEvaluatedKey === undefined) return [];
            return await getPaginatedFilteredVisibleTable(tableName, pageSize, pageNumber - 1, arg, value, items.LastEvaluatedKey);
        }

        if (items.LastEvaluatedKey === undefined) return items.Items;
        return [...items.Items,
            ...(await getPaginatedFilteredVisibleTable(tableName, pageSize, pageNumber, arg, value, items.LastEvaluatedKey, (limit || pageSize) - items.Count))
        ];
    }
...
const getPaginatedTable = async(tableName, pageSize, pageNumber, lastKey) => {
    ...
    if (pageNumber > 0) {
        if (items.LastEvaluatedKey === undefined) return [];
        return getPaginatedTable(tableName, pageSize, pageNumber - 1, items.LastEvaluatedKey);
    }
...
const filterVisible = async(tableName, arg, value) => (DBON ?
    await docClient
    .scan({
        TableName: tableName,
        ExpressionAttributeValues: {
